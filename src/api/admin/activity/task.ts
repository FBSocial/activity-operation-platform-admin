 ...
 */
export function saveTask(activityId: number, activityTaskId: number, params: SaveTaskParams): Promise<void> {
    return http({
        url: `/admin/task/${activityId}/save/${activityTaskId}`,
        method: 'put',
        data: params,
    });
}
 ...
 */
export function getGuildChannels(guildId: string): Promise<ChannelInfo[]> {
    return http<ChannelInfo[]>({
