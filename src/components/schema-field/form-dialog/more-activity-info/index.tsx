...
const ConfigurableMoreActivityInfo = observer(({ value, buttonName, onChange, showTooltip }: ConfigurableShareFieldProps) => {
        ...
        <div>
          <Space>
            <Tooltip title={showTooltip ? (field.description ?? '') : ''} placement="bottom">
              <Button type="default" onClick={handleOpenModalAdd} icon={<PlusOutlined />} disabled={items.length >= 3}>
                {buttonName || '配置'}
              </Button>
            </Tooltip>
            <Space>
