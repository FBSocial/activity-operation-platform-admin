...
function ActivityTemplateCreateOrEdit() {
  ...
  const [baseInfoHasSaveSuccess, setBaseInfoHasSaveSuccess] = useState(false)
  ...
  const handleOpenPreviewDrawer = () => {
    responsiveIframeViewer(previewActivityUrl(routeQuery?.activity_id ?? ''), '预览H5活动效果')
  }
      ...
      try {
        if (form) {
          ...
          if (sectionKey === LotteryTemplateSection.BasicInfo.key) {
            ...
            console.log('%c Line:148 🌽 activityId', 'color:#ed9ec7', activityId)
            if ((routeQuery?.action === 'create' || routeQuery?.action === 'copy') && !activityId) {
              console.log('%c Line:148 🍆 activityId', 'color:#4fff4B', activityId)
              // 创建没有活动 ID
            ...
            setBaseInfoHasSaveSuccess(true)
  ...
  const formEditInit = useCallback(
    (form: Form, data: GetActivityDetail) => {
      ...
      console.log('%c Line:189 🥚 data', 'color:#465975', data, data.online_at)
      ...
      form.setValues(moreActivity)
      form.setValues(bgStyle)
      form.setValues(lotteryPrizeConf)
      form.setValues(taskInfo)
    },
    [setActivityData]
  )
  ...
  }, [navigate, routeQuery.action, routeQuery.activity_id])
          ...
          >
            <div className="flex flex-col gap-y-2">
              ...
              <SectionCard
                ...
                needConfirm={routeQuery?.action === 'create' || routeQuery?.action === 'copy'}
      ...
      {baseInfoHasSaveSuccess && (
        <div className="flex items-center justify-center p-2">
          <Tooltip title="点击预览活动页">
            <Button type="primary" icon={<EyeOutlined />} size="large" danger onClick={handleOpenPreviewDrawer}>
              预览
            </Button>
          </Tooltip>
        </div>
